...
export const calibrate = (v, rho) => {
  ...
  if (vessel.prop_walk_handedness === undefined || vessel.prop_walk_handedness === null) vessel.prop_walk_handedness = null;
  if (vessel.prop_walk_ahead_coeff === undefined || vessel.prop_walk_ahead_coeff === null) vessel.prop_walk_ahead_coeff = 0.0;
  if (vessel.prop_walk_astern_coeff === undefined || vessel.prop_walk_astern_coeff === null) vessel.prop_walk_astern_coeff = 0.0;
  if (vessel.prop_walk_decay_j0 === undefined || vessel.prop_walk_decay_j0 === null) vessel.prop_walk_decay_j0 = 0.0;
  if (vessel.prop_walk_decay_power === undefined || vessel.prop_walk_decay_power === null) vessel.prop_walk_decay_power = 2.0;
  if (vessel.prop_walk_max_ratio === undefined || vessel.prop_walk_max_ratio === null) vessel.prop_walk_max_ratio = 0.0;
  if (vessel.prop_walk_nps_deadband === undefined || vessel.prop_walk_nps_deadband === null) vessel.prop_walk_nps_deadband = 0.0;
  if (vessel.prop_walk_yaw_arm === undefined || vessel.prop_walk_yaw_arm === null) vessel.prop_walk_yaw_arm = 0.0;
...
export class MMGModel {
  ...
  dynamics({ X, psi, delta, h, nps, fl_psi, fl_vel, w_vel, beta_w }) {
    ...
    const { X_P, Y_P, N_P, w_P, K_T, J } = propellerForces(p, current_u, nps, beta_P);
    ...
    const FY = Y_H + Y_R + Y_P + Y_W;
    const FN = N_H + N_R + N_P + N_W;
  ...
  pstep({ X, pos, dT, nps, delta, psi, water_depth, fl_psi, fl_vel, w_vel, beta_w }) {
    ...
    this._updatePivotPoint(new_uvr, new_eta);

    return [new_uvr, new_eta];
  }

  getPivotPoint() {
    return this.lastPivotPoint;
  }

  _updatePivotPoint(uvr, eta) {
    const [, v, r] = uvr;
    if (!Number.isFinite(v) || !Number.isFinite(r) || Math.abs(r) < 1e-6) {
      this.lastPivotPoint = null;
      return;
    }

    const longitudinalOffset = -v / r;
    const [north, east, psi] = eta;
    ...
    this.lastPivotPoint = {
      ...
      world: {
        north: north + Math.cos(psi) * longitudinalOffset,
        east: east + Math.sin(psi) * longitudinalOffset,
      },
    };
  }
}
