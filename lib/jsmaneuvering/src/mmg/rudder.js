...
const FORWARD_PROP_WASH_GAIN = 0.5;
...
export const rudderForces = (p, delta, u, U, beta, r_dash, w_P, J, K_T) => {
    ...
    if (u < 0) {
        ...
        // A coasting vessel still has water moving past the rudder; prop wash
        // only adds to that baseline instead of replacing it.
        u_R = Math.max(0, ambientInflow);

        if (J !== 0) {
            const term1 = 8.0 * K_T / (Math.PI * (J ** 2));
            const sqrt_arg = 1.0 + term1;
            const term2 = 1.0 + p.kappa * (Math.sqrt(Math.max(0, sqrt_arg)) - 1);
            const poweredInflow = ambientInflow * p.epsilon * Math.sqrt(
                p.eta * (term2 ** 2) + (1 - p.eta)
            );
            const propWashBoost = Math.max(0, poweredInflow - u_R);
            u_R += propWashBoost * FORWARD_PROP_WASH_GAIN;
        }
    }
